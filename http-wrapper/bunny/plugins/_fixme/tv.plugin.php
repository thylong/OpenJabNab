...
$Ztamp = $ojnAPI->getApiString("bunny/".$_SESSION['bunny']."/tv/getrfid?".$ojnAPI->getToken());
$Ztamp = "".$Ztamp["value"];
if(!empty($_POST['a']) && (isset($_POST['tvtime']) || isset($_POST['tvchan']))) {
	...
	} else if($_POST['a'] == "setrfid") {
		if(isset($_POST['rfid']) && ( isset($Ztamps[$_POST['rfid']]) || $_POST['rfid'] == '') ) {
			$retour = $ojnAPI->getApiString("bunny/".$_SESSION['bunny']."/tv/setrfid?tag=".$_POST['rfid']."&".$ojnAPI->getToken());
...
<input type="radio" name="a" value="addwbc" checked="true" /> Add Webcast at : <input type="text" name="tvtime" /><br />
<input type="radio" name="a" value="setrfid" /> Set RFID tag : <select name="rfid">
    <option value=""></option>
	<?php foreach($Ztamps as $k=>$v): ?>
	<option value="<?php echo $k; ?>"<?php echo $Ztamp == $k ? ' selected="selected"' : ''?>><?php echo $v; ?> (<?php echo $k; ?>)</option>
	<?php endforeach; ?>
	</select><br />

...
// http://medias.lequipe.fr/logo-rugbyball/1615/20
$reload = false;
if(isset($_POST['addteam'])) {
	$_SESSION['subtab'] = "rugby_team";
	if(strlen(trim($_POST['addteam']))) {
		Message::AddFromApi($ojnAPI->getApiString("bunny/".$_SESSION['bunny']."/rugby/addTeam?team=".urlencode($_POST['addteam'])."&".$ojnAPI->getToken()));
	} else {
	...
	$_SESSION['subtab'] = "rugby_team";
	Message::AddFromApi($ojnAPI->getApiString("bunny/".$_SESSION['bunny']."/rugby/removeTeam?".$ojnAPI->getToken()."&team=".urlencode($_GET['rp'])."&".$ojnAPI->getToken()));
	$reload = true;
}
$teams = array();
$groups = array();
$matchs = array();
$pList = $ojnAPI->getApiList("bunny/".$_SESSION['bunny']."/rugby/getTeams?".$ojnAPI->getToken());
$tList = $ojnAPI->getApiString("plugin/rugby/team?action=list&".$ojnAPI->getToken());
$wList = $ojnAPI->getApiString("plugin/rugby/getMatchs?".$ojnAPI->getToken());
...
if(!isset($_SESSION['subtab']) || !preg_match("|^rugby_|", $_SESSION['subtab'])) {
	$_SESSION['subtab'] = "rugby_team";
}
if($reload)
{
	header("Location: bunny_plugin.php?p=rugby");
	exit();
}
?>
		...
		<ul class="nav nav-tabs">
		  <li<?php echo $_SESSION['subtab'] == 'rugby_team' ? ' class="active"' : '' ?>><a href="#team" data-toggle="tab"><?php echo __tr('Teams') ?></a></li>
		  <li<?php echo $_SESSION['subtab'] == 'rugby_match' ? ' class="active"' : '' ?>><a href="#match" data-toggle="tab"><?php echo __tr('Matchs') ?></a></li>
		</ul>
		<br />
		
			<div class="tab-content">
				<div class="tab-pane<?php echo $_SESSION['subtab'] == 'rugby_team' ? ' active' : '' ?>" id="team">
...
?>
	<tr<?php echo $i++ % 2 ? " class='l2'" : "" ?>>
		...
		<td class="span1"><a class="btn btn-danger" href="bunny_plugin.php?p=rugby&rp=<?php echo $item ?>"><?php echo __tr("Remove") ?></a></td>
	</tr>
<?php } ?>
</table>
<?php
...
?>
				...
				<div class="tab-pane<?php echo $_SESSION['subtab'] == 'rugby_match' ? ' active' : '' ?>" id="match">
...
// http://medias.lequipe.fr/logo-rugbyball/1615/20
*/
$js = '<link href="js/select2.css" rel="stylesheet"/><script src="js/select2.js"></script> <script>function format(team) {if (!team.id) return team.text; return "<img class=\'flag\' src=\'http://medias.lequipe.fr/logo-rugby/" + team.id.toLowerCase() + "/20\'/>" + team.text; }; $(document).ready(function() { $("#teams").select2({formatResult: format,formatSelection: format}); });</script>';

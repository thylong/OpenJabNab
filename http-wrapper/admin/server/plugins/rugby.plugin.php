...
{
	Message::AddFromApi($ojnAPI->getApiString("plugin/rugby/config?action=debug&set=".$_POST['debug']."&".$ojnAPI->getToken()));
	$reload = true;
}
if($reload)
{
	header("Location: server_plugin.php?p=rugby");
	exit();
}
include('include/message.php');
$debug = $ojnAPI->getApiValue('plugin/rugby/config?action=debug&'.$ojnAPI->getToken());
?>

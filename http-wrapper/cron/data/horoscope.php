...
// Horoscoop day RSS	: http://feeds.astro-share.nl/daghoroscoop.rss.php
// Week	: http://feeds.astro-share.nl/weekhoroscoop.rss.php 
// Month	: http://feeds.astro-share.nl/maandhoroscoop.rss.php
...
if(true) {
	$d = array();
	$url = 'http://feeds.astro-share.nl/daghoroscoop.rss.php';
	$xml = simplexml_load_string(file_get_contents($url));
	$channel = (array)($xml->channel);
	foreach($channel['item'] as $i => $item) {
		$desc = preg_replace('/\s*<br.*$/', '', (string)$item->description);
	...
	addToDatas('nl', 'daily', $d);
}
// Weekly NL
if(true) {
	$d = array();
	$url = 'http://feeds.astro-share.nl/weekhoroscoop.rss.php';
	$xml = simplexml_load_string(file_get_contents($url));
	$channel = (array)($xml->channel);
	foreach($channel['item'] as $i => $item) {
		$desc = preg_replace('/\s*<br.*$/', '', (string)$item->description);
	...
	addToDatas('nl', 'weekly', $d);
}
// Monthly NL
if(true) {
	$d = array();
	$url = 'http://feeds.astro-share.nl/maandhoroscoop.rss.php';
	$xml = simplexml_load_string(file_get_contents($url));
	$channel = (array)($xml->channel);
	foreach($channel['item'] as $i => $item) {
		$desc = preg_replace('/\s*<br.*$/', '', (string)$item->description);
...
$data .= '</xml>';
file_put_contents('/home/prod/OpenJabNab/server/bin/data/horoscope.xml', $data);
?>

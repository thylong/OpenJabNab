...
PLUGIN_API_CALL(PluginWeather::Api_GetAllCitiesList) {
  ...
  const auto& alive_date = QDateTime::currentDateTime().addDays(-2); // Get all bunnies connected in the last two days
  while(bunny.hasNext())
  {
    bunny.next();
    auto* b = bunny.value();
    if(!( b->HasPlugin(this) &&
          (b->IsConnected()                                                                     // Bunny is connected
              // FIXME: Meh... Add some kind of "ping", maybe it already exists...
            || b->GetGlobalSetting("Last JabberConnection", alive_date.addDays(-1)) >= alive_date     // Bunny last connection was less than two days ago
            || b->GetGlobalSetting("Last JabberDisconnection", alive_date.addDays(-1)) >= alive_date  // Bunny last disconnection was less than two days ago
          )
        ))
      continue;

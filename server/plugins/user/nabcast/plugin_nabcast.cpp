...
PLUGIN_BUNNY_API_CALL(PluginNabcast::Api_File)
{
	...
	if(action == "list")
	{
		...
		if(libraryMode & MixedLibrary)
			return new ApiAnswers::List(GetUserDir(bunny)->entryList(QStringList("*.mp3")) + nabcastFolder.entryList(QStringList("*.mp3")));
...
PLUGIN_BUNNY_API_CALL(PluginNabcast::Api_Play)
{
	...
	return new ApiAnswers::Ok(QString("Will now play '%1' for bunny '%3'").arg(hRequest.GetArg("nabcast"), QString(bunny->GetID())));
}
...
PLUGIN_BUNNY_API_CALL(PluginNabcast::Api_AddRFID)
{
	...
	return new ApiAnswers::Ok(QString("Add '%1' for RFID '%2', bunny '%3'").arg(hRequest.GetArg("nabcast"), hRequest.GetArg("tag"), QString(bunny->GetID())));
}

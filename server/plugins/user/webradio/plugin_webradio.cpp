...
PLUGIN_BUNNY_API_CALL(PluginWebradio::Api_Preset)
{
	...
	else if(action == "add")
	{
		...
		return new ApiAnswers::Ok(Translator::tr("Add preset '%1' for radio '%2', bunny '%3'", account).arg(hRequest.GetArg("name"), hRequest.GetArg("url"), QString(bunny->GetID())));
	}
	else if(action == "del")
	{
		QMap<QString, QVariant> list = bunny->GetPluginSetting(GetName(), "Presets", QMap<QString, QVariant>()).toMap();
	...
	else if(action == "setdefault")
	{
		...
		return new ApiAnswers::Ok(Translator::tr("Define '%1' preset webradio as default for bunny '%2'", account).arg(hRequest.GetArg("name"), QString(bunny->GetID())));
	}
	else if(action == "play")
	{
...
PLUGIN_API_CALL(PluginWebradio::Api_PluginPreset)
{
	...
	else if(action == "add")
	{
		...
		return new ApiAnswers::Ok(Translator::tr("Add preset '%1' for radio '%2'", account).arg(hRequest.GetArg("name"), hRequest.GetArg("url")));
	}
	else if(action == "del")
	{
		QMap<QString, QVariant> list = GetSettings("Presets", QMap<QString, QVariant>()).toMap();
...
PLUGIN_BUNNY_API_CALL(PluginWebradio::Api_AddRFID)
{
	...
	return new ApiAnswers::Ok(Translator::tr("Add '%1' for RFID '%2', bunny '%3'", account).arg(hRequest.GetArg("name"), hRequest.GetArg("tag"), QString(bunny->GetID())));
}
...
PLUGIN_BUNNY_API_CALL(PluginWebradio::Api_RemoveRFID)
{
	...
	return new ApiAnswers::Ok(Translator::tr("Remove RFID '%2' for bunny '%3'", account).arg(hRequest.GetArg("tag"), QString(bunny->GetID())));
}
...
PLUGIN_BUNNY_API_CALL(PluginWebradio::Api_PlayUrl)
{
	...
	if(streamWebradio(bunny, hRequest.GetArg("url")))
		return new ApiAnswers::Ok(Translator::tr("Now streaming '%1' on bunny '%2'", account).arg(hRequest.GetArg("url"), QString(bunny->GetID())));
	return new ApiAnswers::Error(Translator::tr("Can't stream '%1' on bunny '%2'", account).arg(hRequest.GetArg("url"), QString(bunny->GetID())));
}
...
PLUGIN_BUNNY_API_CALL(PluginWebradio::Api_AddWebcast)
{
	...
	if(!bunny->GetPluginSetting(GetName(), "Schedules", QMap<QString, QVariant>()).toMap().contains(hTime))
	{
		...
		bunny->SetPluginSetting(GetName(), "Schedules", list);
		return new ApiAnswers::Ok(Translator::tr("Add schedule at '%1' to bunny '%2'", account).arg(hRequest.GetArg("time"), QString(bunny->GetID())));
	}
	return new ApiAnswers::Error(Translator::tr("Schedule at '%1' already exists for bunny '%2'", account).arg(hRequest.GetArg("time"), QString(bunny->GetID())));
}
...
PLUGIN_BUNNY_API_CALL(PluginWebradio::Api_RemoveWebcast)
{
	...
	if(list.contains(time))
	{
		...
		return new ApiAnswers::Ok(Translator::tr("Remove schedule at '%1' for bunny '%2'", account).arg(hRequest.GetArg("time"), QString(bunny->GetID())));
	}
	return new ApiAnswers::Error(Translator::tr("No schedule at '%1' for bunny '%2'", account).arg(hRequest.GetArg("time"), QString(bunny->GetID())));
}

...
void PluginWizzflux::analyse(QNetworkReply* networkReply)
{
	if (!networkReply->error()) {
		...
		if(bunny) {
			...
			{
				LogError(QString("Wizz.cc can't send %2 to bunny %1 (%3)").arg(QString(bunny->GetID()), message, QString(bunny->GetXmppResource())));
			}
		}
	}
...
PLUGIN_BUNNY_API_CALL(PluginWizzflux::Api_AddWebcast)
{
    ...
    {
        Cron::RegisterDaily(this, Cron::mkTime(hTime), bunny, Cron::Classic, QVariant::fromValue(hRequest.GetArg("name")));

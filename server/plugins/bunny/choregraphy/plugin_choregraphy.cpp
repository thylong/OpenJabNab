...
PLUGIN_BUNNY_API_CALL(PluginChoregraphy::Api_Config)
{
	...
	else if(action == "add")
	{
		...
		if(!hRequest.HasArg("chor"))
			return new ApiManager::ApiError(Translator::tr("Missing argument '%1' for plugin %2", account).arg("chor", GetName()));
		...
		return new ApiManager::ApiOk(Translator::tr("Added choregraphy '%1' for bunny '%2'", account).arg(chor, QString(bunny->GetID())));
	}
	else if(action == "del")
	{
		if(!hRequest.HasArg("sender"))
		...
		{
			bunny->SetPluginSetting(GetName(), "Choregraphy", list);
			return new ApiManager::ApiOk(Translator::tr("Removed choregraphy '%1' for bunny '%2'", account).arg(sender, QString(bunny->GetID())));
		}
		return new ApiManager::ApiError(Translator::tr("Choregraphy '%1' not found for bunny %2", account).arg(sender, QString(bunny->GetID())));
	}
	else
	{

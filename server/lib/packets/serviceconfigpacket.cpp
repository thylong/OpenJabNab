...
ServiceconfigPacket::ServiceconfigPacket(QByteArray const& m):serviceconfig(m) {}

ServiceconfigPacket * ServiceconfigPacket::Parse(QByteArray const& buffer)
{
	ServiceconfigPacket * p = new ServiceconfigPacket();
	...
	for(int i = 1; i < buffer.size(); i++)
	{
		...
		p->serviceconfig.append(currentChar);
	}
	return p;
}

QByteArray ServiceconfigPacket::GetInternalData() const
{
	QByteArray tmp;
	...
	{
		unsigned char currentChar = serviceconfig.at(i);

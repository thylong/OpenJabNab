...
ChorconfigPacket::ChorconfigPacket() {}

ChorconfigPacket::ChorconfigPacket(QByteArray const& m):chorconfig(m) {}

ChorconfigPacket * ChorconfigPacket::Parse(QByteArray const& buffer)
{
	ChorconfigPacket * p = new ChorconfigPacket();
	...
	for(int i = 1; i < buffer.size(); i++)
	{
		...
		p->chorconfig.append(currentChar);
	}
	return p;
}

QByteArray ChorconfigPacket::GetInternalData() const
{
	QByteArray tmp;
	// Obfuscating algorithm by Sache
	unsigned char previousChar = 35;
	tmp.append((char)0x00);
	for(int i = 0; i < chorconfig.size(); i++)
	{
		unsigned char currentChar = chorconfig.at(i);
